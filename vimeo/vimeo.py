...
class VimeoBlock(XBlock):
    ...
    def student_view(self, context=None):
        ...
        context = {"self": self, "embed_code": embed_code}
    ...
    def get_embed_code_for_url(self, url):
        ...
        if hostname == "vimeo.com":
            ...
            return hostname, "<p>Please add Vimeo video</p>"

        try:
            vimeo_response = requests.get(oembed_url, params=params)
            vimeo_response.raise_for_status()
        except Exception as e:
            return (
                hostname,
                "<p>Error getting video from provider ({error})</p>".format(error=e),
            )
        response = vimeo_response.json()
